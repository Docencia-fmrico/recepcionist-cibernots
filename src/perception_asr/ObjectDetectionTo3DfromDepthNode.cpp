...
#include "perception_asr/ObjectDetectionTo3DfromDepthNode.hpp"
...
ObjectDetectionTo3DfromDepthNode::ObjectDetectionTo3DfromDepthNode()
: Node("objectdetection_to_3d_from_depth_node")
{
  ...
  sync_->registerCallback(std::bind(&ObjectDetectionTo3DfromDepthNode::callback_sync, this, _1, _2));

  info_sub_ = create_subscription<sensor_msgs::msg::CameraInfo>(
    "camera_info", 1, std::bind(&ObjectDetectionTo3DfromDepthNode::callback_info, this, _1));
  detection_pub_ = create_publisher<vision_msgs::msg::Detection3DArray>(
    "output_detection_3d", rclcpp::SensorDataQoS().reliable());
}

void
ObjectDetectionTo3DfromDepthNode::callback_info(sensor_msgs::msg::CameraInfo::UniquePtr msg)
{
...
void
ObjectDetectionTo3DfromDepthNode::callback_sync(
  ...
  const vision_msgs::msg::Detection2DArray::ConstSharedPtr & detection_msg)
{
  ...
  if (detection_pub_->get_subscription_count() > 0) {
    ...
    if (!detections_3d_msg.detections.empty()) {
      RCLCPP_INFO(get_logger(), "PUNTOS DE SILLAS PUBLICADOS");
